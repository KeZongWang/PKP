...
function CradCreate:getsuit(self.id)  --花色
	...
	elseif self.id >= 14 and self.id <= 26 then
		bigType = Cardsuittype.HONG_TAO
	elseif self.id >= 27 and self.id <= 39 then
		bigType = Cardsuittype.MEI_HUA
	elseif self.id >= 40 and self.id <= 52 then
		bigType = Cardsuittype.FANG_KUAI
	elseif self.id == 53 then
		bigType = Cardsuittype.XIAO_WANG
    elseif self.id == 54 then
    	bigType = Cardsuittype.DA_WANG
	end
	return bigType
end
